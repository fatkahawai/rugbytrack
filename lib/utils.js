...
/* TODO: remove this DEBUG code ?    
    // have to create and save a dummy user so we have something to find and clean out, else find() hangs and times out
    var user = new Model({
      userName : 'dummy',
      fullName : 'Remove Me',
      email : 'remove_me@xyz.com'
    });
    user.save( function(err){ 
      if (err) { 
        log.debug("lib.utils.cleanDb: DEBUG error"); 
        return; 
      } 
      log.debug("lib.utils.cleanDb: DEBUG saved a dummy user ok");
    });
    
    Model.find({},function(err,users) {
      log.debug("lib.utils.cleanDb: Model.find() callback: entry");
      if (err) { throw err; }
      log.debug("lib.utils.cleanDb: Model.find() callback: found userName= "+users[0].userName);
    });
/* TODO: remove this DEBUG code above */    
  ...
  etag: function(data) {
    var hash;
    log.debug("lib.utils.eTag: entry");
